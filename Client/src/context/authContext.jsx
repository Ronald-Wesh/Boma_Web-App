...
export const AuthProvider=({children})=>{//children=components that will be wrapped
  ...
  const [loading,setLoading]=useState(true);//tracks loading state=used when checking authentification
  ...
  useEffect(()=>{//runs once when app starts
    const initializeAuth=async()=>{
      ...
      if(token &&userData){//if yes-user is logged in
        try{
...
const login=async(email,password)=>{
  try{
    ...
    toast.success(`Welcome to Boma ${finalUserData.name||finalUserData.email.split('@')[0]}!`);
...
const updateUser=(updatedUserData)=>{//no backend call,just updates local state
